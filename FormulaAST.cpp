    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<double(Position)> args) const override {
                ...
                if (type_ == '+') {result = lhs_->Evaluate(args) + rhs_->Evaluate(args);}
                else if (type_ == '-') {result = lhs_->Evaluate(args) - rhs_->Evaluate(args);}
                else if (type_ == '*') {result = lhs_->Evaluate(args) * rhs_->Evaluate(args);}
                else {
                    if (std::abs(rhs_->Evaluate(args) - 0) < LOWER_NUMBER_BOUND) {
                        throw FormulaError(FormulaError::Category::Div0);
                    }
                    result = lhs_->Evaluate(args) / rhs_->Evaluate(args);
                }
                if (std::abs(result) > UPPER_NUMBER_BOUND)
                {
                    throw FormulaError(FormulaError::Category::Div0);
                }
                if (std::abs(result) < LOWER_NUMBER_BOUND)
                {
                    return 0.;
                }
                return result;
